...
## Publication follow-through (2026-09-09)

Version 1.1.0 is published for all 25 framework packages. The annotated `v1.1.0` tag points to `b57e124de38163bdd9f4c4cdc2683ecff912010f`, which includes the complete remote main history through the white-paper merge. Main was synchronized with that commit before tagging.

All 25 downloaded crates match their official registry checksums and are byte-for-byte identical to the clean committed archives exercised by the release-time consumer tests. Their Git provenance identifies the same release commit. The [published checksum manifest](https://github.com/vicotrbb/nidus/releases/download/v1.1.0/nidus-1.1.0-artifacts.json) records every package. All 25 docs.rs package pages were verified available.

The release commit passed [CI](https://github.com/vicotrbb/nidus/actions/runs/34360448003), [security](https://github.com/vicotrbb/nidus/actions/runs/34360447916), [release integrity](https://github.com/vicotrbb/nidus/actions/runs/34360447896) and [website deployment](https://github.com/vicotrbb/nidus/actions/runs/34360447853). The [publication workflow](https://github.com/vicotrbb/nidus/actions/runs/34363361672) completed successfully, including its fresh public-registry consumer verification. The live website release page and exact white-paper PDF bytes were checked separately.

The post-publication main follow-up changes only the two standalone example lockfiles and this publication record. The lock refresh replaces 18 candidate archive checksums with the published checksums; package versions and dependency graphs remain unchanged. Both source applications passed `cargo test --locked` and exact-version registry-provenance checks. The release tag remains fixed at the source commit used by the published archives.
