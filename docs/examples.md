...
```toml
nidus = { package = "nidus-rs", version = "1.0.5", features = ["http"] }
nidus-sqlx = { version = "1.0.5", features = ["sqlite", "health", "observability"] }
nidus-cache = { version = "1.0.5", features = ["health", "observability"] }
nidus-testing = "1.0.5"
```
...
Before `1.0.5` is published to crates.io, use the pre-publish proof mode:
...
the current local `1.0.5` crates without adding path dependencies to the
