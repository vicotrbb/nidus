...
emits matching `$ref` values and placeholder object entries in
`components.schemas` so the document is internally resolvable. Use the runtime
`OpenApiDocument::schema::<T>()` builder path when the output needs field-level
DTO schemas generated by `utoipa::ToSchema`.
