...
impl OpenApiDocument {
    ...
    pub fn schemas_from_route_metadata(mut self, routes: &[RouteMetadata]) -> Self {
        for route in routes {
            self = self
                .with_schema_registrar(route.request_schema_registrar())
                .with_schema_registrar(route.response_schema_registrar());
        }
        self
    }
    ...
    where
        T: ToSchema,
    {
        self.schemas = self.register_schemas(Self::collect_openapi_schemas::<T>());
        self
    }

    fn collect_openapi_schemas<T: ToSchema>() -> Vec<(String, Value)> {
    ...
    fn with_schema_registrar(mut self, registrar: Option<OpenApiSchemaRegistrar>) -> Self {
        let Some(registrar) = registrar else {
            return self;
        };
        let mut schemas = Vec::new();
        registrar(&mut schemas);
